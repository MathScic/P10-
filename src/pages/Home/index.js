...
const Page = () => {
  ...
  if (!data || !Array.isArray(data?.events)) {
    return <div>Les données ne sont pas dans le format attendu.</div>;
  }

  const currentMonthEvent = data.events.filter((event) => {
    const eventDate = new Date(event.date);
    return eventDate.getMonth() === new Date().getMonth();
  });

  const oneCurrentMontEvent = currentMonthEvent[0];
        ...
        <div className="col presta">
          ...
          {oneCurrentMontEvent && ( // fait apparaitre photo en fonction du mosi de l'event (ou mois le plus proche)
            <EventCard
              ...
              title={oneCurrentMontEvent.title}
              date={new Date(oneCurrentMontEvent.date)}
              small
              label="boom"
            />
          )}
